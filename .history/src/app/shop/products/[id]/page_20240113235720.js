...
const page = ({ params }) => {
  ...
  const addToCart = () => {
    ...
    const isExist =
      old_cart?.filter(
        (dt) =>
          dt?.product_id === product?._id && dt?.email === currentUser?.email
      )?.length > 0;
    ...
    if(!isExist){
        ...
        alert("This Product Already Added On Cart")
    }
  };
  const unit = 50;
  if (unit >= 50 && unit < 100) {
      ...
      <section className="container mx-auto mt-10 shadow-lg pb-5">
        <div className="grid grid-cols-2">
          <div className="flex items-center w-full">
            <div
              className={`image-container relative w-auto max-w-[604px] overflow-hidden  bg-stone-100 rounded-md ${
                zoom ? "zoomed" : ""
              }`}
              // style={{
