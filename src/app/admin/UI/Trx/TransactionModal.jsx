...
const TransactionModal = ({ isOpen, onClose, onSuccess, trx, products }) =>
{
  ...
  filterSort={(optionA, optionB) => {
    // First option (empty) should always stay at the top
    if (optionA.value === "") return -1;
    if (optionB.value === "") return 1;
    
    // Sort other options alphabetically by label (product name)
    return (optionA?.label || '').toLowerCase().localeCompare(
      (optionB?.label || '').toLowerCase()
    );
  }}
  filterOption={(inputValue, option) => {
    // Skip the empty option from search
    ...
    // Check if the product category includes the input value
    // Check if the product description includes the input value
    const descriptionMatches = (option.description || '')
      .toLowerCase()
      .includes(lowerCaseInputValue);
